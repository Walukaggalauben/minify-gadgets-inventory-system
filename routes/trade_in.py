...
trade_in_bp = Blueprint(
    "trade_in",
    __name__,
    url_prefix="/trade-ins"
)
...
def create():
    ...
    if request.method == "POST":
        ...
        customer_data = {
            ...
            "alternative_phone":
                request.form.get("alternative_phone"),
            ...
            "trade_in_date":
                datetime.strptime(
                    request.form.get("trade_in_date"),
                    "%Y-%m-%d"
                ).date(),
        ...
        battery_healths = request.form.getlist(
            "battery_health[]"
        )

        screen_conditions = request.form.getlist(
            "screen_condition[]"
        )

        back_conditions = request.form.getlist(
            "back_condition[]"
        )

        frame_conditions = request.form.getlist(
            "frame_condition[]"
        )

        camera_conditions = request.form.getlist(
            "camera_condition[]"
        )

        charging_ports = request.form.getlist(
            "charging_port[]"
        )

        speaker_statuses = request.form.getlist(
            "speaker_status[]"
        )

        microphone_statuses = request.form.getlist(
            "microphone_status[]"
        )

        face_id_statuses = request.form.getlist(
            "face_id_status[]"
        )

        fingerprint_statuses = request.form.getlist(
            "fingerprint_status[]"
        )

        wifi_statuses = request.form.getlist(
            "wifi_status[]"
        )

        bluetooth_statuses = request.form.getlist(
            "bluetooth_status[]"
        )

        sim_statuses = request.form.getlist(
            "sim_status[]"
        )

        offered_values = request.form.getlist(
            "offered_value[]"
        )

        final_trade_values = request.form.getlist(
            "final_trade_value[]"
        )
        ...
        for i, variant_id in enumerate(variant_ids):
            ...
            items.append({
                ...
                "wifi_status":
                    wifi_statuses[i] if i < len(wifi_statuses) else "",

                "bluetooth_status":
                    bluetooth_statuses[i] if i < len(bluetooth_statuses) else "",

                "sim_status":
                    sim_statuses[i] if i < len(sim_statuses) else "",
                ...
                "box_received":
                    f"box_received_{i}" in request.form,

                "charger_received":
                    f"charger_received_{i}" in request.form,

                "cable_received":
                    f"cable_received_{i}" in request.form,

                "adapter_received":
                    f"adapter_received_{i}" in request.form,

                "earphones_received":
                    f"earphones_received_{i}" in request.form,

                "case_received":
                    f"case_received_{i}" in request.form,

                "screen_protector":
                    f"screen_protector_{i}" in request.form
